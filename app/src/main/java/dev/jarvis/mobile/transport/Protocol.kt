...
/**
 * С какого курсора просить ленту, чтобы собрать картину заново.
 *
 * Не с нуля. Кольцо узла держит две тысячи событий, и на машине, где реально
 * работали, `since = 0` означает «начало запрошенного уже вытеснено» — узел
 * честно отвечает `gap` с пустым списком. Клиент, начавший с нуля, видит не
 * «все сессии», а «ни одной», и наполняется только тем, что случится дальше:
 * сессия, которая молча ждёт ответа, не появится вообще никогда.
 *
 * Просим ровно доступный отрезок: `cursor - buffered`.
 */
fun Hello.replayFrom(): Long = (cursor - buffered).coerceAtLeast(0)
